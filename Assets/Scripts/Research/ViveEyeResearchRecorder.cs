...
public class ViveEyeResearchRecorder : MonoBehaviour
{
    ...
    private void Start()
    {
        if (startAutomatically && hasSessionConfig)
            StartRecording();
    }
    ...
    public void StartRecording()
    {
        ...
        {
            Debug.LogError(
                "[EyeResearch] 尚未設定 session metadata。" +
                "請先呼叫 ConfigureSession(participantId, sessionId)" +
                "再 StartRecording()。未建立 CSV。");
            return;
        }
        ...
        string fileName =
            $"{safeParticipant}_{safeSession}_" +
    ...
    private void RecordOneSample(double currentTime)
    {
        ...
        AddField(row, FormatDouble(GetNetworkTimeSeconds()));
        AddField(row, EscapeCsv(GetExperimentPhaseName()));
    ...
    private void WriteHeader()
    {
        writer.WriteLine(
            ...
            "timestamp_utc,network_time_s,experiment_phase,unity_frame,sample_index," +
            "gaze_api_success," +
    ...
    private string GetExperimentPhaseName()
    {
        if (experimentFlowManager != null &&
            experimentFlowManager.IsSpawned)
        {
            // e.g. DailyDiscussion_Part1 / StoryNarration_Practice
            return experimentFlowManager.PhaseAndPartLabel;
        }
