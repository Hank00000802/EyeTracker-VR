...
$remote = "/sdcard/Android/data/com.hank.viveeyetracking/files/EyeTrackingData"
#這裡接不同VR會需要改，建議開兩資料夾儲存不同VR的資料 VR_A or VR_B
$local  = "$HOME\Desktop\EyeTrack\ViveEyeStudyData\VR_B"

#如果 local路徑 不存在 → 建立資料夾
New-Item -ItemType Directory -Path $local -Force | Out-Null

#取得 VR 裡所有 CSV
$remoteFiles = & $adb shell "ls -1 $remote/*.csv 2>/dev/null"

#一個一個處理 VR 裡的 CSV，獲取新的CSV
foreach ($remoteFile in $remoteFiles)
{
    ...
    $localFile = Join-Path $local $fileName

    if (-not (Test-Path $localFile))
    {
        Write-Host "[NEW] Downloading $fileName"
